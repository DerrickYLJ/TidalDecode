...
#         keys = past_key_value[0][:, :, 0, :, :]
#         values = past_key_value[0][:, :, 1, :, :]
...
#             # print(f"pure_search: {time.time() - before_search}")
...
#             keys_retrieved = keys[:, :, i_index, :].reshape(-1, head_dim)[sorted_I_k].reshape(bsz, -1, self.top_k, head_dim)
#             values_retrieved = values[:, :, i_index, :].reshape(-1, head_dim)[sorted_I_k].reshape(bsz, -1, self.top_k, head_dim)
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        #         None, self.head_dim, self.num_key_value_heads, self.top_k, 0
        #     )
        #     key_states_cpu = key_states.cpu()
