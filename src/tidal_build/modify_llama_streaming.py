...
def llama_tidal_attention_forward(
    ...
    **kwargs,
    ...
    if self.layer_idx < sparse_layer_start or q_len == kv_seq_len:
        ...
        token_budget = min(last_dim_size, top_k) # token budget controls sparsity level, limits number of tokens attended to, balances efficiency vs performance
        # in standard attention, each token attends to ALL previous tokens
        # with token budget, each token attends to ONLY top-k most relevant tokens
        # so token budget is top k --> will stay the same for streaming LLM
    
        # decoding 
        if (self.layer_idx == sparse_layer_start or self.layer_idx == correction_layer) and top_k >= last_dim_size: # if there is no need to do sparse attention
            self.pos_dict = torch.ones_like(attn_weights)
        elif self.layer_idx == sparse_layer_start or self.layer_idx == correction_layer:
            ...
            # assert recent_start == attention_sink + middle_budget
            recent_start = last_dim_size-most_recent_amount # total seq len - most_recent_amount
            ...
            recent_indices = torch.arange(recent_start, last_dim_size, device = attn_weights.device)
            recent_indices = recent_indices.expand(attn_weights.shape[:-1] + (most_recent_amount,))
        
            # combine indices
            top_k_indices = torch.cat([sink_indices, middle_indices, recent_indices], dim=-1)
            
            # _, top_k_indices = torch.topk(attn_weights, k=token_budget, dim=-1) # get the top k of the token budget
            top_k_mask = torch.zeros_like(attn_weights).scatter_(-1, top_k_indices, 1.0) # puts a 1.0 at the indices in tok_k --> mask out the non-top k tokens
...
# @artij: modifies a pre-training Llama model to use Tidal attention mechanisms, replaces standard attention with custom tidal attention
def enable_llama_tidal_attention(
    model, top_k, attn_type="tidal", sparse_layer_start=2, correction_layer=9, attn_sink=0, most_recent_scale=1):
    def wrap_forward(module): # @artij: creates a new forward function that uses tidal attention
        ...
        def new_tidal_forward(
            ...
            most_recent_scale=most_recent_scale,
            **kwargs,
        ):
    ...
    for name, module in reversed(model._modules.items()):
        if len(list(module.children())) > 0:
            enable_llama_tidal_attention(module, top_k, attn_type, sparse_layer_start, correction_layer, attn_sink, most_recent_scale)
        global layer_id
