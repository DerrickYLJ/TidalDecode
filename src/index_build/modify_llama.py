...
class KeyValueIndexStore:
    def __init__(self, dimension, num_kv_heads, top_k, past_key_value, layer_idx):
    ...
    def update_kv_cache(self, key_states, value_states):
        ...
        self.past_key_value[1] = torch.cat(
            [self.past_key_value[1], value_states], dim=2
        )
    ...
    def search_index_store(self, query_states):
        ...
        for head in range(num_heads):
            ...
            keys_retrieved = (
                self.past_key_value[0][:, i_index, :, :]
                .reshape(-1, head_dim)[sorted_I_k]
                .reshape(bsz, self.top_k, head_dim)
            )
            values_retrieved = (
                self.past_key_value[1][:, i_index, :, :]
                .reshape(-1, head_dim)[sorted_I_k]
                .reshape(bsz, self.top_k, head_dim)
            )
        ...
        retrieved_k = retrieved_k.view(num_heads, bsz, self.top_k, head_dim).transpose(
            0, 1
        )
        retrieved_v = retrieved_v.view(num_heads, bsz, self.top_k, head_dim).transpose(
            0, 1
        )
...
def llama_index_build_attention_forward(
    ...
    if past_key_value is not None:
        ...
        past_key_value = (
            self.kv_index_store.past_key_value[0],
            self.kv_index_store.past_key_value[1],
        )
    else:
        self.kv_index_store = KeyValueIndexStore(
            ...
            top_k,
            [key_states_cpu, value_states_cpu],
            self.layer_idx,
        )
